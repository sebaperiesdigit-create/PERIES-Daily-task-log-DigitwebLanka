...
Each qualifying loan-request email must create or update one record and, after a successful extraction, send one automatic acknowledgement reply in the same Gmail thread.
...
10. Add an acknowledgement-reply workflow. For a successfully extracted qualifying request, prepare/send one reply to the original sender in the same Gmail thread using an approved, configurable template and the extracted request details.
11. Track the outgoing reply privately using the source email ID and sent-message ID so reruns never send a second acknowledgement.
12. Do not send an automatic reply for non-qualifying, incomplete, ambiguous, malformed, duplicate, corrected, or cancelled requests until the approved workflow explicitly allows it.
...
- Exact acknowledgement-reply wording, language, sender identity, and whether all extracted details should appear in the reply
- Exact send trigger: immediately after a valid extraction, only after a staff review, or another approved approval stage
...
7. Tests proving:
   ...
   - a valid request produces one correctly addressed, thread-linked acknowledgement draft/output;
   - rerunning the same request cannot produce a second acknowledgement;
   - ineligible or review-required requests never produce an acknowledgement.
8. A configurable acknowledgement-reply template and safe test/draft mode. Use placeholders for the approved details; do not invent final wording.
9. Safe setup and run instructions.
10. A concise list of exact information still needed before live Gmail activation and automatic reply sending.

## Completion boundary

Do not activate scheduled syncing, connect production Gmail, send automatic acknowledgements, or write to a production database without explicit approval. Keep changes focused on this task and report changed files plus test evidence.
