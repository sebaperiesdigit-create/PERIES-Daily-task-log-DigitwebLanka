...
## The `new-joiner-guide` skill (built)

The deliverable skill from this task, built via `skill-builder` after a full `grill-me` interrogation. Same skill identity (`name: new-joiner-guide` in every variant's frontmatter) shipped as **three installable packages** — install exactly one per environment, never more than one into the same skills scope:

| Variant | Path | Use in |
|---|---|---|
| Modular | `.claude/skills/new-joiner-guide/` | Claude Code only — thin routing-engine `SKILL.md` + `reference/*.md` topic files, loaded on demand |
| Standalone | `.claude/skills/new-joiner-guide-standalone/` | Claude Chat or Claude Cowork only — one complete self-contained `SKILL.md`, zero file dependency |
| Universal | `.claude/skills/new-joiner-guide-universal/` | Any environment / a joiner who moves between surfaces — the standalone `SKILL.md` plus the modular package's `reference/*.md` as optional (never required) deeper-dive material |

**Trigger phrases:** "I am new" / "how do I start" / "I received a task" / "I finished my work" / "I can't access X" or "I don't understand" / "continue from where I stopped" / "I need to take over a task" / "I need to log today's work" / "where do I publish this". Both auto-invocable and `/new-joiner-guide`.

**What it does:** identifies the joiner's stage first, then routes through the real guide's setup/task-workflow/publishing/daily-log/takeover/closure process one step at a time — handing over the guide's own paste-into-ChatGPT prompt templates rather than planning tasks itself (GPT=Brain/Claude Code=Worker is preserved throughout). Escalates restricted/unclear items to the guide's named owners (Varmens/Sajeesan/Tamil Selvan/MD) instead of guessing.

**Deliverables** (project-record docs, kept separate from the installable packages): `deliverables/usage-and-install-notes.md`, `deliverables/traceability-note.md`, `deliverables/test-scenarios-and-results.md`, `deliverables/unresolved-guide-ambiguities.md`.
