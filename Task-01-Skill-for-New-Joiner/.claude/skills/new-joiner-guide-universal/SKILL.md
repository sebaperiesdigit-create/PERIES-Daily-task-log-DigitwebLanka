...
<!--
UNIVERSAL package (1 of 3 distributions of the same "new-joiner-guide" skill).
This SKILL.md is fully self-contained -- everything needed to run the skill is
inline below, and it works standalone with zero local file/tool/connector access,
exactly like new-joiner-guide-standalone. It additionally ships reference/*.md
files (same content as the new-joiner-guide modular package) as OPTIONAL
deeper-dive material for environments that do have file access (Claude Code,
some Claude Cowork setups) -- the skill never depends on them being present or
being read; they're a convenience for going deeper on one topic, not a
requirement. Install this variant when the joiner may use more than one Claude
surface (e.g. Chat sometimes, Code other times) and you want one install that
works everywhere.
...
# New Joiner Guide (Universal, self-contained + optional deeper reference)

An interactive coach that walks a Mini-AIOS new joiner through the real company guide — setup, task workflow, publishing, daily logging, task takeover, and closure. This file is complete on its own: everything needed is inline below, nothing else needs to be read or fetched. Where file access exists, `reference/*.md` alongside this file has the same content in more depth per topic — treat it as optional background, never a dependency. It routes and explains; it does not plan tasks on its own.
...
*(deeper reference, optional: `reference/setup.md`, `reference/business-glossary.md`)*
...
*(deeper reference, optional: `reference/task-workflow.md`)*
...
*(deeper reference, optional: `reference/task-workflow.md`, `reference/takeover-and-closure.md`)*
...
*(deeper reference, optional: `reference/escalation-and-governance.md`)*
...
*(deeper reference, optional: `reference/takeover-and-closure.md`)*
...
*(deeper reference, optional: `reference/daily-log.md`)*
...
*(deeper reference, optional: `reference/database-and-publishing.md`)*
...
*(deeper reference, optional: `reference/folders-and-conventions.md`)*
...
- `reference/*.md` alongside this file is optional deeper background, kept in sync with this file's content — treat any difference between them as a sign the two have drifted and flag it rather than trusting one over the other silently.
